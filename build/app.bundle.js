...
var _Item = __webpack_require__(331);
...
var Items = {
  ...
  sortArray: function sortArray(arr) {
    ...
    arr.sort(function (a, b) {
      var sorted = self.compare(b.done, a.done) || self.compare(a.createdDate, b.createdDate);
      return sorted;
    });
    return arr;
  },
  getItem: function getItem(id) {
    var itemToGet = void 0;
  ...
  addItem: function addItem(input, isImportant) {
    var id = this.randomId();
    var createdDate = Items.getDate();
    var newItem = new _Item2.default(input, id, createdDate, isImportant);
    this.data.items.push(newItem);
    return newItem;
  },
  getItems: function getItems() {
    var items = this.sortArray(this.data.items);
    return this.data.items;
  },
  setItems: function setItems(fetcheditems) {
    var items = this.sortArray(fetcheditems);
    this.data.items = items;
  },
  updateItem: function updateItem(item, text, date) {
  ...
  isItemDone: function isItemDone(item) {
    return item.done;
  },
  isItemEditing: function isItemEditing(item) {
    return item.editing;
  },
  markItemDoneUndone: function markItemDoneUndone(itemToMark) {
    this.data.items.forEach(function (item) {
...
var _items2 = __webpack_require__(126);
...
var addItemSubmit = function addItemSubmit(e) {
  e.preventDefault();
  var isImportant = _ui2.default.getImpValue();
  var input = _ui2.default.getInputValue();
  if (input.trim() !== '') {
    var newItem = _items3.default.addItem(input, isImportant);
    _ui2.default.addItemToList(newItem);
    _storage2.default.storeItem(newItem);
    _ui2.default.clearInput();
  }
}; // export only public, not private(_)

// save to firebase or create db with api

var itemBtnSubmit = function itemBtnSubmit(e) {
  e.preventDefault();
  var itemId = e.target.parentNode.parentNode.parentNode.id.split('-')[1];
  var item = _items3.default.getItem(itemId);
  if (e.target.id === 'editBtn') {
    var itemIsDone = _items3.default.isItemDone(item);
    if (itemIsDone) {
      return;
    }
    var input = _ui2.default.setItemToEdit(itemId);
    input.addEventListener('blur', function () {
      var dateToUpdate = _items3.default.getDate();
      var textToUpdate = _ui2.default.updateEditedItem(item, dateToUpdate);
      var itemToUpdate = _items3.default.updateItem(item, textToUpdate, dateToUpdate);
      _storage2.default.updateItemStorage(itemToUpdate);
    });
  } else if (e.target.id === 'deleteBtn') {
    _items3.default.deleteItem(item);
    _ui2.default.deleteItem(itemId);
    _storage2.default.deleteItemFromStorage(itemId);
  } else if (e.target.id === 'doneBtn') {
    _items3.default.markItemDoneUndone(item);
    _ui2.default.markItemDoneUndone(itemId);
    _storage2.default.updateItemStorage(item);
    var _items = _items3.default.getItems();
    _ui2.default.fillItemsList(_items);
  }
};

_ui2.default.selectors.addBtn.addEventListener('click', addItemSubmit);
_ui2.default.selectors.regularItemsList.addEventListener('click', itemBtnSubmit);
_ui2.default.selectors.importantItemsList.addEventListener('click', itemBtnSubmit);
var items = _storage2.default.getItemsFromStorage();
_items3.default.setItems(items);
_ui2.default.fillItemsList(items);
...
var Item = function Item(text, id, createdDate, important) {
  _classCallCheck(this, Item);

  this.text = text, this.id = id, this.done = false, this.createdDate = createdDate, this.editedDate = null, this.important = important;
};
...
var _items = __webpack_require__(126);
...
var UI = {
  ...
  fillItemsList: function fillItemsList(items) {
  ...
  setItemToEdit: function setItemToEdit(itemId) {
    var span = document.querySelector('li#item-' + itemId).firstElementChild;
    var html = '<input type="text" value="' + span.innerText + '" id="input-' + itemId + '" class="edit-input"></input>';
    span.innerText = '';
    span.insertAdjacentHTML('afterbegin', html);
    span.children[0].focus();
    return span.children[0];
  },
  updateEditedItem: function updateEditedItem(item, editedDate) {
    var input = document.querySelector('#input-' + item.id);
    ...
    input.remove();
    var editedDateSpan = document.querySelector('#item-' + item.id).children[1].children[1].children[0];
    editedDate = this.formatDate(editedDate);
    editedDateSpan.innerText = 'edited: ' + editedDate;
    return inputValue;
  },
  deleteItem: function deleteItem(itemId) {
    var li = document.querySelector('li#item-' + itemId);
    li.remove();
  },
  markItemDoneUndone: function markItemDoneUndone(itemId) {
    var li = document.querySelector('li#item-' + itemId);
